...
const resUsers = users.map((user, index) => {
	const isPercent = index % 2;
	return {
		...user,
		nip: Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15),
		kelasJabatan: isPercent ? 'Kelas Jabatan 1' : 'Kelas Jabatan 2',
		golongan: isPercent ? 'Golongan 1' : 'Golongan 2',
		pangkat: isPercent ? 'Pangkat 1' : 'Pangkat 2',
		tmtJabatan: isPercent ? 'TMT Jabatan 1' : 'TMT Jabatan 2',
		angkaKredit: isPercent ? 50 : 100,
		jenisKelamin: isPercent ? 'Laki-Laki' : 'Perempuan',
		thnLulus: isPercent ? 2003 : 2008,
		jenjang: isPercent ? 'S1' : 'S2',
		jurusan: isPercent ? 'Teknik Informatika' : 'Sistim Informatika',
		agama: isPercent ? 'Islam' : 'Kristen',
		tempatLahir: isPercent ? 'Jakarta' : 'Surabaya',
		tglLahir: '15 Feb 1990',
		pengangkatan: '15 Feb 2020',
		bup: 'BUP',
		age: getRandomArbitrary(20, 55),
		category: isPercent ? 'PNS' : 'PPNPN',
	};
});

// GET DATA
mock.onGet('/api/users/list').reply(200, resUsers);
