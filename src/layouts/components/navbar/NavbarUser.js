		...
		const renderCartItems = this.state.shoppingCart.map(item => {
			return (
				<div className="cart-item" key={item.id}>
					<Media className="p-0" onClick={() => history.push('/ecommerce/product-detail')}>
						...
						<Media className="overflow-hidden pr-1 py-1 pl-0" body>
							...
							<span className="item-desc font-small-2 text-truncate d-block">{item.desc}</span>
						...
						return (
							<Dropdown
								...
								<DropdownMenu right>
									...
									<DropdownItem tag="a" onClick={e => context.switchLanguage('id')}>
